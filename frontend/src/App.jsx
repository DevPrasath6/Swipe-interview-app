...
function InterviewSession() {
  ...
  const handleSubmitAnswer = () => {
    ...
    // Score based on answer content - give 0 for empty answers
    let score = 0;
    if (currentAnswer.trim().length > 0) {
      // Simple scoring based on answer length and completeness
      const wordCount = currentAnswer.trim().split(/\s+/).length;
      if (wordCount >= 10) {
        score = Math.floor(Math.random() * 30) + 70; // Good answer: 70-100
      } else if (wordCount >= 5) {
        score = Math.floor(Math.random() * 20) + 50; // Adequate answer: 50-70
      } else {
        score = Math.floor(Math.random() * 30) + 20; // Poor answer: 20-50
      }
    }
  ...
  const completeInterview = () => {
    // Fix scoring to handle empty answers
